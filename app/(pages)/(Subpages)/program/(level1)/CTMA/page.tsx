...
export default function CTMA() {
  const tabData = [
	...
	const ListLeft = [
		{ title: "ACAMS는 왜 CTMA를 개발했는가?", desc: "거래 모니터링은 금융 범죄를 탐지하는데 도움이 되는 지속적인 고객실사의 중요한 부분입니다. 고객 온보딩 후의 거래 모니터링은 회사가 비정상적이거나 의심스러운 활동을 식별하는 핵심 과정입니다. 거래 모니터링시스템에서 발생하는 alert는 반드시 조사해야 하며, 필요한 경우 KoFIU에 신고해야 합니다. 강력한 거래 모니터링 프로그램을 구축하는 것은 완전한 자금세탁방지 프로그램 수립을 위해 핵심적인 부분이며, 따라서 alert을 처리하고 조사하는 담당자를 잘 교육하는 것이 필수적입니다." },
		{ title: "CTMA는 왜 Associate 레벨인가?", desc: "경력의 다양한 레벨을 지원하기 위해 ACAMS는 최대 약 1년 정도의 경험이 있는 개인을 대상으로 하는 Associate 레벨의 자격증을 제공합니다. 이 자격증은 경력 자격 요건이 없기 때문에 금융 범죄 예방 분야를 처음 접하는 분에게도 유용합니다." },
		{ title: "왜 CTMA 자격증을 획득해야 하는가?", desc: "거래 감시는 AML 및 금융 범죄 컴플라이언스 프로그램의 핵심 구성요소입니다. 개인의 경우, CTMA는 거래 감시 절차에 대한 지식을 인증함으로써 차별화할 수 있습니다. 기관의 경우, CTMA 자격증이 있는 직원을 보유함으로써 갈수록 복잡해지는 법률, 규정 및 정책 환경에서 AML 및 금융 범죄 컴플라이언스에 대한 강력한 의지를 표명할 수 있습니다." },
		{ title: "CTMA 자격증은 누구를 위한 것인가?", desc: "CTMA는 미래에 거래 모니터링 업무를 희망하거나 현재 해당 업무를 담당하고 있는 초기 경력 전문가, 그리고 해당 직원들을 감독하거나 지원하는 담당자를 위한 자격증입니다. CTMA자격증 보유 및 6~12개월 경력이 있는 담당자는 거래 모니터링시스템에서 alert이 발생한 시점부터 alert을 해제하거나 의심스러운 활동임을 판단 및 상부 보고하는 시점까지 alert을 조사할 수 있어야 합니다. 기술 전문가들이 CTMA 프로그램 수강을 원할 수도 있지만, 이 프로그램은 관련 기술을 심도 있게 다루지 않습니다. TM 경보를 발생하는 기술은 임계값, 오탐(false positives) 이유와 같은 핵심 원리를 이해하는 수준에서만 다뤄집니다." },
	];
	const ListRight = [
		{
			title: '조직을 위한 혜택',
